...
class ReconAutomation {
  ...
  def calculateTotalRecordCount(sourceDF: DataFrame, targetDF: DataFrame, alias: String): DataFrame = {
    try {
      /**
       * Making sure that the column name and column count are same in source and target
       */
...
object ReconAutomationObject {
  ...
  def main(args: Array[String]): Unit = {
     ...
     **/
    val sourceDF = new ReconAutomation().readDataAndConvertToDataframe(readType,fileType,sourcePath,
      sourceDatabase, sourceTable, sourceUser, sourcePassword, sourceDriverName, sourceConnString)
    println("Source Data:")
    sourceDF.show()

    val targetDF = new ReconAutomation().readDataAndConvertToDataframe(readType,fileType,targetPath,
